...
const generateSearchText = (title, description, body) => {
...
router.get("/", async (req, res) => {
    try {
        ...
        const whereClause = {};
        if (topicId)
            whereClause.topicId = String(topicId);
        if (courseId)
...
router.get("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const content = await db_1.prisma.content.findUnique({
            where: { id },
...
router.post("/", (0, validateRequest_1.validateRequest)(content_schema_1.createContentSchema), async (req, res) => {
    try {
        ...
        let slug = req.body.slug || (0, slugify_1.default)(title, { lower: true, strict: true });
        slug = (0, slugify_1.default)(slug, { lower: true, strict: true });
        // Clean HTML if blog
        const cleanBody = type === "BLOG" && body ? (0, sanitize_html_1.default)(body, {
        ...
        const existing = await db_1.prisma.content.findFirst({
        ...
        const content = await db_1.prisma.content.create({
            data: {
                topicId,
                courseId,
                title,
...
router.put("/:id", (0, validateRequest_1.validateRequest)(content_schema_1.updateContentSchema), async (req, res) => {
    try {
        ...
        const current = await db_1.prisma.content.findUnique({ where: { id }, include: { contentTags: true } });
        if (!current)
            return res.status(404).json({ error: "Content not found" });
        ...
        if (data.body && current.type === "BLOG") {
            data.body = (0, sanitize_html_1.default)(data.body, {
                allowedTags: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'p', 'a', 'ul', 'ol', 'li', 'strong', 'em', 'code', 'pre', 'img'],
                allowedAttributes: { 'a': ['href'], 'img': ['src', 'alt'] }
            });
        }
        ...
        const content = await db_1.prisma.content.update({
            where: { id },
            data: {
                ...data,
...
router.delete("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.prisma.content.delete({
            where: { id },
        });
...
router.post("/reorder", (0, validateRequest_1.validateRequest)(content_schema_1.reorderContentSchema), async (req, res) => {
    try {
        ...
        await db_1.prisma.$transaction(contentIds.map((id, index) => db_1.prisma.content.update({
            where: { id },
...
router.post("/upload/presign", async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            warning: "⚠️ Presigned URL generation not fully implemented. Use S3 SDK for production.",
        });
    }
    catch (error) {
...
router.post("/upload/video", async (req, res) => {
    try {
        ...
        return res.status(200).json({
            uploadUrl,
            warning: "⚠️ Stream presigned URL generation not fully implemented. Make authenticated request to Cloudflare API.",
